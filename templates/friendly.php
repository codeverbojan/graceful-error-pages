<?php
/**
 * Friendly error page template.
 *
 * Warm, encouraging design with an inline SVG illustration,
 * generous whitespace, pill-shaped buttons, and softer corners.
...
<body class="gcep-error-page gcep-template-friendly" data-dark-mode="<?php echo esc_attr( (string) ( $context['dark_mode'] ?? 'auto' ) ); ?>">
	<main class="gcep-card">
		...
		<?php else : ?>
			<svg class="gcep-illustration" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
				<circle cx="36" cy="36" r="36" fill="var(--gcep-brand-color)" opacity="0.08"/>
				<circle cx="36" cy="36" r="24" fill="var(--gcep-brand-color)" opacity="0.1"/>
				<path d="M36 22v16" stroke="var(--gcep-brand-color)" stroke-width="3" stroke-linecap="round"/>
				<circle cx="36" cy="46" r="2" fill="var(--gcep-brand-color)"/>
			</svg>
		<?php endif; ?>
